...
class CPTEnv(Env):
    ...
    def __init__(self,iWorld, full_observable=True, penalty=-3.0, r_catch=20, max_steps=20, step_cost = 0.0):
        ...
        self.CPT_params = {'reference': 0, 'lamb': 1, 'gamma_gain': 1, 'gamma_loss': 1,
                           'delta_gain': 1, 'delta_loss': 1, 'agentk': 1}
        self.set_CPT(**self.CPT_params)
        self.seed()

    ######################
    def step(self, agents_action,is_joint=False):
        assert (self._step_count is not None), \
            "Call reset before using step method."

        if is_joint:
            aR = int(agents_action[0]/ 5)
            aH = int(agents_action[1] % (5-1))
            assert aR <= 4, f'robot action {aR} out of range'
            assert aH <= 4, f'human action {aH} out of range'
            agents_action = [aR,aH]
        ...
        for agent_i, action in enumerate(agents_action):
            if not self._done: self.__update_agent_pos(agent_i, action)

        # # Get stage reward
        for agent_i in range(self.n_agents):
            is_CPT = (self.CPT_params['agentk'] == agent_i)
            p_pen = self._CPT_p_penalty if is_CPT else self._p_penalty
            r_pen = self._CPT_penalty if is_CPT else self._penalty
            r_catch = self._CPT_r_catch if is_CPT else self._r_catch

            if self.is_penalty(self.agent_pos[agent_i]) and self.enable_penalty:
                rewards[agent_i] += p_pen * r_pen
            if self._prey_caught(self.prey_pos):
                rewards[agent_i] += r_catch + (-1*self._step_count)
        ...
        if self.f_scaleR is not None: rewards = self.f_scaleR(rewards)
    ...
    def seed(self, n=None):
        ...
        return [seed]
    def set_CPT(self,reference, gamma_gain, gamma_loss, lamb, delta_gain, delta_loss,agentk=1):
        # for key in kwargs.keys():
        #     assert key in self.CPT_params.keys(), f'Loaded Unknown CPT param {key}'
        #     self.CPT_params[key] = kwargs[key]

        self.CPT_params['reference'],   b   = reference, reference
        self.CPT_params['gamma_gain'],  yg  = gamma_gain, gamma_gain
        self.CPT_params['gamma_loss'],  yl  = gamma_loss, gamma_loss
        self.CPT_params['lambda'],      lam = lamb, lamb
        self.CPT_params['delta_gain'],  dg  = delta_gain, delta_gain
        self.CPT_params['delta_loss'],  dl  = delta_loss, delta_loss
        self.CPT_params['agentk']           = agentk

        p_pen = self._p_penalty
        r_pen = self._penalty
        r_catch = self._r_catch

        if (r_pen - b) >= 0:
            rhat_pen = pow(r_pen-b,yg)
            phat_pen = pow(p_pen,dg)/pow(pow(p_pen,dg)+pow(1-p_pen,dg),dg)
        else:
            rhat_pen = -lam*pow(abs(r_pen-b),yl)
            phat_pen = pow(p_pen, dl) / pow(pow(p_pen, dl) + pow(1 - p_pen, dl), dl)

        if (r_catch - b) >= 0: rhat_catch = pow(r_catch - b, yg)
        else: rhat_catch = pow(r_catch - b, yl)

        self._CPT_p_penalty = phat_pen
        self._CPT_penalty   = rhat_pen
        self._CPT_r_catch   = rhat_catch
    ...
    def __update_agent_pos(self, agent_i, move):
        ...
        if move == 0:  # down
            ...
            raise Exception(f'Action {move} Not found!')
