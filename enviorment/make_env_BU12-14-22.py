...
class CPTEnv(Env):
    ...
    def __init__(self,iWorld, full_observable=True, penalty=-3.0, prey_capture_reward=20, max_steps=20, step_cost = 0.0):
        ...
        #self.scale_reward={'gain':1, 'power':1 }

        self.CPT_params = {'b':0,'gamma_gain':1,'gamma_loss':1,'delta_gain':1,'delta_loss':1,'agentk':1}
    ...
    def step(self, agents_action,is_joint=False):
        ...
        for agent_i in range(self.n_agents):
            if self.is_penalty(self.agent_pos[agent_i]) and self.enable_penalty:
                rewards[agent_i] += self._p_penalty * self._penalty
        ...
        #     if self.is_penalty(self.agent_pos[agent_i]) and self.enable_penalty:
        #
        #         if self.CPT_params['agentk'] == agent_i:
        #             p_pen = self._CPT_p_penalty
        #             r_pen = self._CPT_r_penalty
        #         else:
    ...
    def seed(self, n=None):
        ...
        return [seed]
    def set_CPT(self,**kwargs):
        for key in kwargs.keys():
            assert key in self.CPT_params.keys(), f'Loaded Unknown CPT param {key}'
            self.CPT_params[key] = kwargs[key]

        b = self.CPT_params['reference']
        yg = self.CPT_params['gamma_gain']
        yl = self.CPT_params['gamma_loss']
        lam = self.CPT_params['lambda']
        dg = self.CPT_params['delta_gain']
        dl = self.CPT_params['delta_loss']
        ...
        if (r_pen - b) >= 0:
            ...
            p_pen
        else: rhat_pen = -lam*pow(abs(r_pen-b),yg)




        self.CPT_dynamics['r_penalty'] =
