...
class Joint_CPTEnv(Env):
    ...
    def __init__(self,iWorld, full_observable=True, penalty=-3.0, prey_capture_reward=20, max_steps=20, step_cost = 0.0):
        ...
        self.n_SoloActions = 5
        _A = list(np.arange(self.n_SoloActions))
        self.SoloAction_a2name = np.array(['DOWN','LEFT','UP','RIGHT','NOOP'],dtype=object)
        self.SoloAction_a2move = np.array( [[1,0], [0,-1], [-1,0], [0,1], [0,0]])

        self.JointAction_a2iagent = np.array(list(itertools.product(_A,_A)))
        self.JointAction_asolo2jointlist = np.empty([n_agents, self.n_SoloActions],dtype=object)
        for k in range(n_agents):
            for a in range(self.n_SoloActions):
                idxs = list(np.array(np.where(self.JointAction_a2iagent[:, k] ==a)).flatten())
                self.JointAction_asolo2jointlist[k,a] = idxs

        self.JointAction_a2name = np.array(list(itertools.product(*[self.SoloAction_a2name,self.SoloAction_a2name])))
        self.JointAction_a2move = np.array(list(itertools.product(*[self.SoloAction_a2move,self.SoloAction_a2move])))
        self.n_JointActions = len(self.JointAction_a2iagent)
        ...
        self.action_space = MultiAgentActionSpace([spaces.Discrete(self.n_JointActions) for _ in range(self.n_agents)])
    ...
    def step(self, agents_action):
        agents_action = [int(a) for a in agents_action]
    ...
    def _prey_decide_action(self):
        ...
        _prey_move_probs = np.zeros(self.n_SoloActions)
        for action_i in range(self.n_SoloActions):
            _move = action_i
            _pos = self.__next_pos(agent_i=3, curr_pos = self.prey_pos, ia=_move)
    ...
    #         return [self.JointAction_a2name for i in range(self.action_space[agent_i].n)]
    #     else:
    #         return [[self.JointAction_a2name[i] for i in range(ac.n)] for ac in self.action_space]
    ...
    def get_agent_obs(self):
        _obs = []
        for agent_i in range(self.n_agents):  _obs += list(self.agent_pos[agent_i])
        _obs += list( self.prey_pos)
        return [_obs for _ in range(self.n_agents)]

    def __update_agent_pos(self, agent_i, ia):
        ia = int(ia)
        curr_pos = np.array(copy.copy(self.agent_pos[agent_i]))
        move = self.JointAction_a2move[ia][agent_i]
        ...
        if self.is_valid(next_pos):
            ...
            # if agent_i == 0: print(f'valid_agent0_state')


    def __next_pos(self,agent_i, curr_pos, ia):
        ia = int(ia)
        if agent_i==3: move = self.SoloAction_a2move[ia]
        else:  move = self.JointAction_a2move[ia][agent_i]
        next_pos = curr_pos + move
        return next_pos

    def __update_prey_pos(self, prey_pos, ia):
        curr_pos = np.array(prey_pos)
        assert ia < self.n_SoloActions, '__update_prey_pos using joint actions'

        if not self._done:
            move = self.SoloAction_a2move[ia]
            next_pos = curr_pos + move
