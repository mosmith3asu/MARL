...
class CPTEnv(Env):
    ...
    def __init__(self, prey_move_probs=(0.175, 0.175, 0.175, 0.175, 0.3),
                 full_observable=True, penalty=-3.0, prey_capture_reward=20, max_steps=20, step_cost = 0.0):

        self.dist_cost = 0.0
        n_agents = 2
        self.nActions = 5

        # self._start_obs = [[1,2],[5,5],[3,3]] #
        # self._start_obs = [[1, 1], [1, 1], [1, 1]]

        grid_shape = (7, 7)
        agent_view_mask = grid_shape#(5, 5)
        self._walls = []
        self._walls += [[0, c] for c in range(grid_shape[1])]
        self._walls += [[r, 0] for r in range(grid_shape[0])]
        self._walls += [[grid_shape[1] - 1, c] for c in range(grid_shape[1])]
        self._walls += [[r,grid_shape[0] - 1] for r in range(grid_shape[0])]
        self._walls += [[2,2],[2,4],[4,2],[4,4]]

        # self._start_obs = [[5, 2], [1, 2], [4, 1]]
        # self._penalty_states = [[1, 1], [2, 1], [3, 1], [3,4],[3,5]]
        self._start_obs = [[5, 3], [1, 3], [5, 1]]
        self._penalty_states = [[1, 1],[1, 2], [2, 1], [3, 1], [3, 4], [3, 5]]
        ...
        # # [Box([0. 0.], [1. 1.], (2,), float32), Box([0. 0.], [1. 1.], (2,), float32)]
        # self.observation_space = [Box(low = [0., 0.], high = [1., 1.]),
        #                           Box(low = [0., 0.], high = [1., 1.])]
        # self.state = [[0.0, 0.17], [0.0, 0.83]]
        # self.total_moves = 20
        ...
        self.action_space = MultiAgentActionSpace([spaces.Discrete(25) for _ in range(self.n_agents)])
    ...
    def step(self, agents_action):
        ...
        for agent_i in range(self.n_agents):
            if self.is_penalty(self.agent_pos[agent_i]):
                # rewards[agent_i] += np.random.choice([0,self._penalty],p=[0.5,0.5])
        ...
        if self._prey_caught(self.prey_pos):
            ...
            for agent_i in range(self.n_agents):
                ...
                rewards[agent_i] -= self._step_count
        else: # Move Prey
    ...
    def render(self, mode='human'):
        ...
        write_cell_text(img, text='prey', pos=self.prey_pos, cell_size=CELL_SIZE, fill='white', margin=0.4)
        img = np.asarray(img)
    ...
    ######################
    def _prey_decide_action(self,lam= 1):
    ...
    def __draw_base_img(self):
        ...
        for wall in self._walls:
            fill_cell(self._base_img, wall, cell_size=CELL_SIZE, fill=WALL_COLOR, margin=0.0)
        for pen in self._penalty_states:
            fill_cell(self._base_img, pen, cell_size=CELL_SIZE, fill=PEN_COLOR, margin=0.0)
    ...
    #     is_wall = (np.array(pos) in np.array(self._walls))
    #     return is_wall
...
JOINT_ACTION_ID = np.array(list(itertools.product(*[np.arange(5),np.arange(5)])))
