...
def run(run_config):
    ...
    lr_scheduler = None#LambdaLR(optimizer, lr_lambda=[lambda epoch: epi_lr[epoch] for _ in range(len(optimizer.param_groups))])
    ...
    for episode_i in range(max_episodes):
        ...
        if memory.size() > warm_up_steps:
            train(q, q_target, memory, optimizer, gamma, batch_size, update_iter,lr_scheduler=lr_scheduler)
        ...
        if time2report:
            ...
            print("lr : {:<10}".format(round(lr, 4)), end='')
