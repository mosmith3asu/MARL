...
class QNet(nn.Module):
    ...
    def add_exploration(self,q,sp,directed = True):
        q = q.flatten()
    ...
    def sample_NR_action(self, obs, sp,rationality,epsilon):
        """
        ...
        if np.random.rand() <= epsilon:
            ...
            _qAH = self.add_exploration(QsA[0, 1].detach(), sp=sp)
            _qAR = self.add_exploration(QsA[0, 0].detach(), sp=sp)
    ...
    def sample_SW_action(self, obs, sp,rationality,epsilon):
        """Sample social welfare action"""
        QsA = self.forward(obs)
        # qAR = self.add_exploration(QsA[0, 0].detach(), sp=sp)  # 1 x n_actions
        # qAH = self.add_exploration(QsA[0, 1].detach(), sp=sp)  # 1 x n_actions
    ...
    def sample_action(self, obs, sp=None,rationality=None,epsilon=None,choose_best=False):
        # Load Defaults
        ...
        if self.sample_method in ['NR','NR_max']:
            action = self.sample_NR_action( obs, sp = sp, rationality=rationality,epsilon=epsilon)
        elif self.sample_method == 'SW':
            action = self.sample_SW_action(obs,  sp = sp, rationality=rationality, epsilon=epsilon)
...
#
#
#     def sample_action(self, obs, sp=None,rationality=None,epsilon=None,best=False):
#         # Load Defaults
#         if rationality is None: rationality = self.rationality
#         if sp is None: sp = self.sp
#         if epsilon is None: epsilon = self.epsilon
...
def test(env, num_episodes, q):
    ...
    for episode_i in range(num_episodes):
        ...
        while not done:
            action = q.sample_action(torch.Tensor(state).unsqueeze(0),sp=0.0,epsilon=0.0,rationality=None)
...
def run(run_config):
    ...
    optimizer = optim.Adam(q.parameters(), lr=start_lr)
    ...
    epi_rationality = np.hstack([warmup_lamb, improve_lamb, perform_lamb])

    warmup_lr = start_lr * np.ones(full_explore_steps)
    perform_lr = end_lr * np.ones(full_exploit_steps)
    improve_lr = schedule(start_lr, end_lr, n_improve_epi, slope=improve_slope)
    epi_lr = np.hstack([warmup_lr, improve_lr, perform_lr])
    ...
    for episode_i in range(max_episodes):
        ...
        if time2report:
            ...
            print("lr : {:<10}".format(round(optimizer.lr,4)), end='')
...
if __name__ == "__main__":
    ...
    config = {
        'iWorld':    1,
        'r_scale':   1, # None
        'lr':        [0.005,0.00001], # increase if learns no policy decrease if unstable rewards
        'gamma':     0.90,
        'lambda':    [10, 10],
        'sp':        [4.0, 0.01], 'epsilon':   [0.0, 0.0],
        #'sp':        [0.0, 0.0], 'epsilon':   [1.0, 0.1],
        'action_sampler': 'NR_max',
        'max_episodes': max_episodes,
        'no_penalty_steps': 0,
        'warm_up_steps': 2000,
