...
class QNet(nn.Module):
    ...
    def sample_action(self, obs, w_explore,rationality=100,epsilon=0.0):

        np.array(list(itertools.product(*[np.arange(5),np.arange(5)])))
        ...
        #aJ = np.random.choice(np.arange(25))
            ...
            # SOCIAL WELFARE TEST ################
            # Q_SW = QsA[0, 0] + QsA[0, 1]
            # aJ = torch.argmax(Q_SW)
            # action = aJ * torch.ones((QsA.shape[0], QsA.shape[1],))

            # -------------------------------------
            # qAH = QsA[0, 1].detach().reshape([5, 5])
            # qAR = QsA[0, 0].detach().reshape([5, 5])
            # aH = torch.argmax(torch.sum(qAH, dim=0)) #aH = torch.argmax(torch.sum(qAH, dim=1))
            # aR = torch.argmax(torch.sum(qAR, dim=1)) #aR = torch.argmax(torch.sum(qAR, dim=0))
            # aJ = self.n_solo_actions * aR + aH  # joint action
            # action = aJ * torch.ones((QsA.shape[0], QsA.shape[1],))
            # -------------------------------------
            qAH = QsA[0, 1].detach()
            qAR = QsA[0, 0].detach()

            phhat_H =torch.sum(ijoint[1] * 1 / self.n_solo_actions, dim=0)
            phhat_R =torch.sum(ijoint[0] * 1 / self.n_solo_actions, dim=0)
            qhat_H = qAH * phhat_R #torch.sum(qAH * phhat_H, dim=0)
            qhat_R = qAR * phhat_H #torch.sum(qAR * phhat_R, dim=0)
            # aH = torch.argmax(qhat_H)  # dim=1
            # aR = torch.argmax(qhat_R)  # dim=0

            phat_H = torch.special.softmax(qhat_H,dim=0)
            phat_R = torch.special.softmax(qhat_R,dim=0)
            qH = qAH * phat_R
            qR = qAR * phat_H
            aH = torch.argmax(qH)  # dim=1
            aR = torch.argmax(qR)  # dim=0





            # qhhat_H = torch.sum(qAH * phhat_H, dim=0)
            # qhhat_R = torch.sum(qAR * phhat_R, dim=0)
            # aH = torch.argmax(qhhat_H)  # dim=1
            # aR = torch.argmax(qhhat_R)  # dim=0

            # aH = torch.argmax(qAH)  # dim=1
            # aR = torch.argmax(qAR)  # dim=0
            ...
            # QsA = self.forward(obs) # 1 x n_agents x n_actions
            #
            # qAH = QsA[0, 1].detach().reshape([5, 5]) # qAH = self.add_exploration(QsA[0, 1].detach(),w_explore=w_explore).reshape([5,5]) # 1 x n_actions
            # qAR = QsA[0, 0].detach().reshape([5, 5])  # qAR = self.add_exploration(QsA[0, 0].detach(),w_explore=w_explore).reshape([5,5]) # 1 x n_actions
            #
            ...
            # qhhat_H = torch.matmul(phhatA_R.T, qAH)
            # qhhat_R = torch.matmul(qAR, phhatA_H.T)
            # phatA_H = torch.special.softmax(lambdaH * qhhat_H, dim=1)
            # phatA_R = torch.special.softmax(lambdaR * qhhat_R, dim=0)
            #
            # # Get probability of ego transition then joint transition
            # qhat_H = torch.matmul(phatA_R.T, qAH)
            # qhat_R = torch.matmul(qAR, phatA_H.T)
            # pA_H = torch.special.softmax(lambdaH * qhat_H, dim=1)
            # pA_R = torch.special.softmax(lambdaR * qhat_R, dim=0)
            #
            # print(f'\r', end='')
            ...
            #
            # # aH = torch.argmax(phatA_H.flatten())  #
            # # aR = torch.argmax(phatA_R.flatten())  #
            # aH = torch.argmax(torch.sum(qAH,dim=1).flatten())
            # aR = torch.argmax(torch.sum(qAR,dim=0).flatten())
            # aJ = (self.n_solo_actions - 1) * aR + aH  # joint action
            # action = aJ * torch.ones((QsA.shape[0], QsA.shape[1],))
