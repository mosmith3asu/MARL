...
def Qlearning(iWorld, lr, gamma, batch_size, buffer_limit, log_interval, max_episodes,
    ...
    for ith_episode in range(max_episodes):
        ...
        eligibility = {
            ...
            'hist': np.zeros([20,2],dtype=object)
       }
        ...
        while not done:
            ...
            #eligibility['trace'][SAi] += 1
            eligibility['hist'][env.step_count-1][0] = 1
            eligibility['hist'][env.step_count-1][1] = SAi
            for agent_i,reward in enumerate(rkj):
                ...
                for et,SAi in eligibility['hist'][:env.step_count]:
                    Qk.Qk[agent_i][SAi] = Qk.Qk[agent_i][SAi] + (lr)*TD_err*(et)

            eligibility['hist'][:,0] *= eligibility['lambda'] * discount_factor
...
class Qfun(object):
    ...
    def sample_action(self,obs_k,epsilon,pd_partner=None):
        if pd_partner is None:
            pd_partner = np.ones([self.num_agents, self.num_actions]) / self.num_actions
        action_k = np.zeros(2)
        Qks = self.at(obs_k)
        for agent_i in range(self.num_agents):
