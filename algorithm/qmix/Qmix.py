...
def main(iWorld, lr, gamma, batch_size, buffer_limit, log_interval, max_episodes,
         max_epsilon, min_epsilon, test_episodes, warm_up_steps, no_penalty_steps,update_iter, chunk_size,
         update_target_interval, recurrent):


    TYPE = 'Baseline'
    Logger.file_name = f'Fig_QMix_{TYPE}'
    Logger.update_save_directory(f'results/QMix/OBP_W{iWorld}/')
    Logger.update_plt_title(f'W{iWorld} QMix Training Results')
    ...
    for episode_i in range(max_episodes):
        ...
        with torch.no_grad():
            ...
            while not done:
                ...
                memory.put((state, action, (np.array(reward)).tolist(), next_state, [int(done)]))
                score += sum(reward)
                state = next_state
        ...
        if episode_i % log_interval == 0 and episode_i != 0:
            ...
            print("\r#{:<10}/{} episodes , avg train score : {:.1f}, test score: {:.1f} n_buffer : {}, eps : {:.1f}"
